...
function transformToLis(obj){
  // Solution code here...
  return Object.keys(obj).map(e => '<li>' + e + ':' + ' ' + obj[e] + '</li>');
}
...
const count = (target, input) => {
  // Solution code here...
  return input.map(e => e.filter(num => num === target).length).reduce((prevValue, curValue) => prevValue + curValue, 0);
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.flat().reduce((prevValue, curValue) => prevValue + curValue);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(e => e.filter(ex => (ex % 5 === 0) && (typeof ex === 'number' )).map(num => Math.pow(2, num)));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(e => e.gender === 'male' || e.gender === 'female').map(el => (el.name)).join(' and ');
};
